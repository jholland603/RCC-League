...
### Stored vs Displayed Scores
Every value in `round_scores` = holes won + win bonus:
- **Winner:** stored = holes won + 1
- **Loser:** stored = holes won + 0
- **Tie:** stored = holes won + 0.5 (each)

Stored scores always sum to **10.0**. Displayed scores always sum to **9.0**.

### Display Adjustment (strip the win bonus)
| Result | Winner display | Loser display |
|--------|---------------|---------------|
| Win/Loss | stored − 1 | stored (no change) |
| Tie | stored − 0.5 | stored − 0.5 |

**Only the winner's score is adjusted in a win/loss. Both are adjusted in a tie.**

### W/L/T and Margin
- Compare stored scores directly: higher = win, equal = tie.
- Margin = winner's holes − loser's holes (winner's holes = stored − 1, loser's holes = stored).
...
1. **Round scores** — add to `round_scores`. Stored = holes won + bonus (winner +1, loser +0, tie +0.5 each). Sum must = 10.0.
